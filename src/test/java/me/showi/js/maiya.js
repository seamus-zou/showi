...
//by lc
eval(function(p,a,c,k,e,r){e=function(c){return c.toString(a)};if(!''.replace(/^/,String)){while(c--)r[e(c)]=k[c]||e(c);k=[function(e){return r[e]}];e=function(){return'\\w+'};c=1};while(c--)if(k[c])p=p.replace(new RegExp('\\b'+e(c)+'\\b','g'),k[c]);return p}('n m(d,e,f){f=f||0;2 g=\'\',6=+e.5(-1),e=e.5(6,e.j);2 a,b,c;3(!f){a=7,b=8,c=o}4{a=8,b=k,c=7}l(2 i=0,9=d.j;i<9;i++){2 h=e.p(d[i]);3(h===a){g+=\'.\'}4 3(h===b||h===c){g+=\'0\'}4{g+=h}}q g}',27,27,'||var|if|else|slice|x|13|12|len||||||||||length|11|for|sm|function|14|indexOf|return'.split('|'),0,{}))
	...
	$('#attend_submit').click(function(){
		...
		$.ajax({
			...
			success:function(result){				
                var icon_heart = "<i class='mia_icons mia_icons2'></i>";    
                // var span_a =  "<span class='sc'></span>"   
                // $("#attend_submit").append(span_a);                                    
                _this.html('收藏').css({'background':'#fff','line-height':'80px'});
                $("#attend_submit").append(icon_heart);  
				if(result.res== false){
...
function ajaxItemInfo(data) {
    ...
    if(typeof data.price != "undefined") {
        data.price = data.sign ? sm(data.price, data.sign) : data.price;
        data.m_price = data.sign ? sm(data.m_price, data.sign, 1) : data.price;
        if(data.price >= 0) {
            $("#item_price").text(data.price);
            $("#mprice").text(data.m_price);
    ...
    if((data.status == 1 && data.o_s == 1) || data.status == 2 || (data.status == 1 && data.o_s == 4)) {
        ...
        if(typeof data.price != "undefined") {
            ...
            var html ='<dt class="tit_txt fz14">规格</dt><dd id="detail_size_list">';
            var si = '';
